	...
	<!-- 페이징 -->
		<div class="text-center">
			<%
				if(searchName == null){			
			%>
					<a class="btn btn-sm btn-outline-success mr-3" href="<%=request.getContextPath()%>/deptemp/deptEmpList.jsp?currentPage=1">처음</a>
					<%
						if(currentPage > 1){
					%>
							<a class="btn btn-sm btn-outline-success mr-3" href="<%=request.getContextPath()%>/deptemp/deptEmpList.jsp?currentPage=<%=currentPage-1%>">이전</a>
					<%
						}
					%>
					...
					%>
							<a class="btn btn-sm btn-outline-success mr-3" href="<%=request.getContextPath()%>/deptemp/deptEmpList.jsp?currentPage=<%=currentPage+1%>">다음</a>
					<%
						}
					%>
					<a class="btn btn-sm btn-outline-success mr-3" href="<%=request.getContextPath()%>/deptemp/deptEmpList.jsp?currentPage=<%=lastPage%>">마지막</a>
			<% 
				}else{			
			%>
					<a class="btn btn-sm btn-outline-success mr-3" href="<%=request.getContextPath()%>/deptemp/deptEmpList.jsp?currentPage=1&searchName=<%=searchName%>">처음</a>
					<%
						if(currentPage > 1){
					%>
							<a class="btn btn-sm btn-outline-success mr-3" href="<%=request.getContextPath()%>/deptemp/deptEmpList.jsp?currentPage=<%=currentPage-1%>&searchName=<%=searchName%>">이전</a>
					<%
						}
					%>
					<span class="btn btn-sm btn-outline-success mr-3"><%=currentPage%></span>
					<%
						if(currentPage < lastPage){
					%>
							<a class="btn btn-sm btn-outline-success mr-3" href="<%=request.getContextPath()%>/deptemp/deptEmpList.jsp?currentPage=<%=currentPage+1%>&searchName=<%=searchName%>">다음</a>
					<%
						}
					%>
					<a class="btn btn-sm btn-outline-success mr-3" href="<%=request.getContextPath()%>/deptemp/deptEmpList.jsp?currentPage=<%=lastPage%>&searchName=<%=searchName%>">마지막</a>
			<% 
				}
			%>
		</div>
